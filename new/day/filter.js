...
//1
const result = numbers.filter((number) => number > 60);
console.log(result);
//2
const grateless = (arr, n1, n2) => {
  const res = arr.filter((number) => number > n1 && number < n2);
  return res;
};
console.log(grateless(numbers, 25, 200));
//3
const movies = [
  {
    id: 1,
...
//4
const raterun = (arr, callback,runt) => {
  const result = [];
  if (typeof callback === "function") {
    for (let i = 0; i < arr.length; i++) {
      if (callback(arr[i], i, arr, 5,runt)) {
        result.push(arr[i]);
      }
    }
    return result;
...
const ratings = (data, i, arr, rating,runt) => {
  return data.rating.average > rating&&data.runtime < runt;;
};
console.log(raterun(movies, ratings,51));
//5
const rateruns = (arr, callback,runt) => {
  const result = [];
  if (typeof callback === "function") {
    for (let i = 0; i < arr.length; i++) {
      if (callback(arr[i], i, arr, 5,runt)) {
        result.push(arr[i].name+","+arr[i].rating.average);
      }
    }
    return result;
  } else {
...
console.log(rateruns(movies, ratings,51));